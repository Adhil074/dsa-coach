...
//     //  Save submission (for weak-area detection) 
//     try {
...
//         verdict: solvedNow ? "correct" : "incorrect",
//         createdAt: new Date(),
//       });
//     } catch (err) {
...
interface SubmitBody {
  ...
  code: string;
  language: string;
  verdict?: "correct_optimal" | "correct_suboptimal" | "incorrect"; // NEW: Accept verdict from frontend
}
...
export async function POST(request: Request) {
  try {
    ...
    const code = String(body.code ?? "");
    const language = String(body.language ?? "javascript");
    const verdict = body.verdict; // NEW: Get verdict from request
    ...
    // Save submission with proper verdict
    try {
      await Submission.create({
        userId: new Types.ObjectId(String(user._id)),
        problemId: new Types.ObjectId(String(problemDoc._id)),
        ...
        topic: String(problemDoc.topic ?? ""),
        difficulty: String(problemDoc.difficulty ?? ""),
        verdict: verdict, // FIXED: Save the actual verdict
        createdAt: new Date(),
      });
    } catch (err) {
      console.error("Submission save error:", err);
